// CSendKeys.cpp: Class Module to achieve VisualBasic's SendKeys functionality in C++.
//
...
if (w == NULL)
{
  ...
  if (WindowTitle)
	_tcscpy_s(titleclass, _tcslen(WindowTitle)+1, WindowTitle);

  titleclass[l1] = 0;

  if (WindowClass)
	_tcscpy_s(titleclass+l1+1, _tcslen(WindowClass)+1, WindowClass);
